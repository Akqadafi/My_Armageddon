...
locals {
  waf_log_resource_arn = try (
    aws_wafv2_web_acl.arcanum_cf_waf01.arn,
    aws_wafv2_web_acl.arcanum_waf01[0].arn
  )
}
...
resource "aws_wafv2_web_acl_logging_configuration" "arcanum_waf_logging01" {
  ...
  resource_arn = aws_wafv2_web_acl.arcanum_cf_waf01.arn

  log_destination_configs = [
    aws_cloudwatch_log_group.arcanum_waf_log_group01[0].arn
  ]

dynamic "redacted_fields" {
  for_each = var.enable_waf_redaction ? [1] : []
  content {
...
dynamic "redacted_fields" {
  ...
  depends_on = [
    ...
    aws_cloudwatch_log_group.arcanum_waf_log_group01]
}
...
# # Explanation: S3 WAF logs are the long-term archive—arcanum likes receipts that survive dashboards.
# resource "aws_s3_bucket" "arcanum_waf_logs_bucket01" {
#   count = var.waf_log_destination == "s3" ? 1 : 0

#   bucket = "aws-waf-logs-${var.project_name}-${data.aws_caller_identity.arcanum_self01.account_id}"
...
# resource "aws_s3_bucket_public_access_block" "arcanum_waf_logs_pab01" {
#   count = var.waf_log_destination == "s3" ? 1 : 0

#   bucket                  = aws_s3_bucket.arcanum_waf_logs_bucket01[0].id
...
# resource "aws_wafv2_web_acl_logging_configuration" "arcanum_waf_logging_s3_01" {
#   count = var.enable_waf && var.waf_log_destination == "s3" ? 1 : 0

#   resource_arn = aws_wafv2_web_acl.arcanum_waf01[0].arn
#   log_destination_configs = [
#     aws_s3_bucket.arcanum_waf_logs_bucket01[0].arn
#   ]

#   depends_on = [aws_wafv2_web_acl.arcanum_waf01]
# }
...
# resource "aws_s3_bucket" "arcanum_firehose_waf_dest_bucket01" {
#   count = var.waf_log_destination == "firehose" ? 1 : 0

#   bucket = "${var.project_name}-waf-firehose-dest-${data.aws_caller_identity.arcanum_self01.account_id}"
...
# # Explanation: Firehose needs a role—arcanum doesn’t let random droids write into storage.
# resource "aws_iam_role" "arcanum_firehose_role01" {
#   count = var.waf_log_destination == "firehose" ? 1 : 0
...
# resource "aws_iam_role_policy" "arcanum_firehose_policy01" {
#   count = var.waf_log_destination == "firehose" ? 1 : 0
#   name  = "${var.project_name}-firehose-policy01"
#   role  = aws_iam_role.arcanum_firehose_role01[0].id
...
#         ]
#         Resource = [
#           aws_s3_bucket.arcanum_firehose_waf_dest_bucket01[0].arn,
#           "${aws_s3_bucket.arcanum_firehose_waf_dest_bucket01[0].arn}/*"
#         ]
#       }
#     ]
...
#     role_arn   = aws_iam_role.arcanum_firehose_role01[0].arn
#     bucket_arn = aws_s3_bucket.arcanum_firehose_waf_dest_bucket01[0].arn
#     prefix     = "waf-logs/"
#   }
# }

# # Explanation: Connect shield generator to conveyor belt—WAF -> Firehose stream.
# resource "aws_wafv2_web_acl_logging_configuration" "arcanum_waf_logging_firehose01" {
