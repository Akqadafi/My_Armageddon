...
############################################
# Security Groups (ALB + ASG)
############################################

# ALB SG: allow inbound HTTP from the internet (or lock to CloudFront later)
resource "aws_security_group" "liberdade_alb_sg01" {
  name        = "${local.name_prefix}-alb-sg01"
  description = "ALB security group"
  vpc_id      = aws_vpc.liberdade_vpc01.id

  tags = { Name = "${local.name_prefix}-alb-sg01" }
}

resource "aws_vpc_security_group_ingress_rule" "liberdade_alb_ingress_http" {
...
# ASG/EC2 SG: ONLY allow inbound from the ALB SG on port 80
resource "aws_security_group" "liberdade_asg_sg01" {
  name        = "${local.name_prefix}-asg-sg01"
  description = "ASG instance security group (only from ALB)"
  vpc_id      = aws_vpc.liberdade_vpc01.id

  tags = { Name = "${local.name_prefix}-asg-sg01" }
}

resource "aws_vpc_security_group_ingress_rule" "liberdade_asg_ingress_http_from_alb" {
  ip_protocol                  = local.tcp_protocol
...
resource "aws_vpc_security_group_egress_rule" "liberdade_asg_egress_all" {
  ip_protocol       = local.all_protocol
  security_group_id = aws_security_group.liberdade_asg_sg01.id
  from_port         = -1
  to_port           = -1
...
resource "aws_lb" "liberdade_alb01" {
  ...
  tags = { Name = "${local.name_prefix}-alb01" }
}
...
resource "aws_lb_target_group" "liberdade_tg01" {
  ...
  tags = { Name = "${local.name_prefix}-tg01" }
}
...
resource "aws_lb_listener_rule" "liberdade_origin_header_rule" {
...
############################################
# IAM for ASG instances (SSM + optional Secrets)
############################################
...
resource "aws_iam_policy" "liberdade_read_tokyo_secret" {
  name        = "${local.name_prefix}-read-tokyo-secret"
  description = "Allow Sao Paulo EC2/ASG to read Tokyo Secrets Manager secret"
  ...
  policy = jsonencode({
    ...
    Statement = [{
      ...
      Resource = "arn:aws:secretsmanager:ap-northeast-1:233781468925:secret:lab3a/rds/mysql-*"
    }]
  })
}

resource "aws_iam_role_policy_attachment" "liberdade_attach_read_tokyo_secret" {
  role       = aws_iam_role.liberdade_ec2_role01.name
  policy_arn = aws_iam_policy.liberdade_read_tokyo_secret.arn
}
...
resource "aws_launch_template" "liberdade_lt01" {
  ...
  # vpc_security_group_ids = [aws_security_group.liberdade_asg_sg01.id]

  user_data = base64encode(file("${path.module}/1a_user_data.sh"))
  ...
  # IMPORTANT: add your Sao Paulo user_data that connects to Tokyo RDS via TGW
  # user_data = base64encode(file("${path.module}/1a_user_data.sh"))

  tag_specifications {
    resource_type = "instance"
    tags          = { Name = "${local.name_prefix}-asg-ec2" }
  }
}
