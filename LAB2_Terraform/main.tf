############################################
# Locals (naming convention: arcanum-*)
############################################
locals {
...
# Explanation: arcanum needs a hyperlane—this VPC is the Millennium Falcon’s flight corridor.
...
# Explanation: arcanum wants the private base to call home—EIP gives the NAT a stable “holonet address.”
...
# Explanation: NAT is arcanum’s smuggler tunnel—private subnets can reach out without being seen.
resource "aws_nat_gateway" "arcanum_nat01" {
  allocation_id = aws_eip.arcanum_nat_eip01.id
  subnet_id     = aws_subnet.arcanum_public_subnets[0].id # NAT in a public subnet
...
# Explanation: Private subnets route outbound internet via NAT (arcanum-approved stealth).
...
# Explanation: EC2 SG is arcanum’s bodyguard—only let in what you mean to.
...
resource "aws_security_group" "arcanum_ec2_sg02" {
...
#   ip_protocol       = local.tcp_protocol
#   security_group_id = aws_security_group.arcanum_ec2_sg02.id
...
# resource "aws_vpc_security_group_egress_rule" "arcanum_ec2_sg_egress_http" {
#   ip_protocol       = local.tcp_protocol
#   security_group_id = aws_security_group.arcanum_ec2_sg01.id
#   from_port         = local.ports_http
#   to_port           = local.ports_http
...
#My working click ops environment
resource "aws_vpc_security_group_egress_rule" "arcanum_ec2_sg_egress_db" {
...
# TODO: student adds inbound MySQL 3306 from aws_security_group.arcanum_ec2_sg01.id
...
resource "aws_vpc_security_group_ingress_rule" "arcanum_rds_sg_ingress_mysql" {
  ...
  referenced_security_group_id = aws_security_group.arcanum_ec2_sg01.id #allow traffic ONLY from specified SG
}
...
resource "aws_db_subnet_group" "arcanum_rds_subnet_group01" {
  ...
  subnet_ids = aws_subnet.arcanum_private_subnets[*].id
...
resource "aws_db_instance" "arcanum_rds01" {
  ...
  vpc_security_group_ids = [aws_security_group.arcanum_rds_sg01.id]
...
# Explanation: arcanum refuses to carry static keys—this role lets EC2 assume permissions safely.
    ...
    "Statement" : [
      {
        ...
        "Resource" : "arn:aws:secretsmanager:${data.aws_region.arcanum_region01.region}:${data.aws_caller_identity.arcanum_self01.account_id}:secret:lab1a/rds/mysql*"
...
# Explanation: Parameter Store is arcanum’s map—endpoints and config live here for fast recovery.
...
# Explanation: Secrets Manager is arcanum’s locked holster—credentials go here, not in code.
...
# Explanation: Metrics are arcanum’s growls—when they spike, something is wrong.
...
# resource "aws_vpc_endpoint" "arcanum_vpce_ssm" { ... }
