...
# Explanation: Static files are the easy win—arcanum caches them like hyperfuel for speed.
resource "aws_cloudfront_cache_policy" "arcanum_cache_static01" {
  ...
  parameters_in_cache_key_and_forwarded_to_origin {
    # Explanation: Static should not vary on cookies—arcanum refuses to cache 10,000 versions of a PNG.
...
# Explanation: APIs are dangerous to cache by accident—arcanum disables caching until proven safe.
...
# Explanation: Origins need context—arcanum forwards what the app needs without polluting the cache key.
resource "aws_cloudfront_origin_request_policy" "arcanum_orp_api01" {
  name    = "arcanum-orp-api01"
  ...
  headers_config {
    ...
    headers {
      items = [
        "Content-Type",
        "Origin",
        # Add your secret header here if you're using origin cloaking:
        "X-Origin-Verify"
      ]
    }
  }
...
# Explanation: Static origins need almost nothing—arcanum forwards minimal values for maximum cache sanity.
...
# Explanation: Make caching intent explicit—arcanum stamps Cache-Control so humans and CDNs agree.
...
# # Explanation: Default behavior is conservative—arcanum assumes dynamic until proven static.
...
# # Explanation: Static behavior is the speed lane—arcanum caches it hard for performance.
